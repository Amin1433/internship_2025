...
import os # Importation ajoutée pour os.makedirs
import numpy as np # Importation ajoutée pour np.save
...
# --- MODIFICATION START (evaluate_mil_model modifiée) ---
def evaluate_mil_model(model, dataloader, device, criterion): # Ajout du paramètre criterion
    model.eval()
    total_correct = 0
    total = 0
    ...
    return avg_acc, avg_loss # Retourne à la fois la précision et la perte
# --- MODIFICATION END ---

# --- MODIFICATION START (train_mil_model modifiée) ---
def train_mil_model(model, train_loader, val_loader, device, n_epochs=10, lr=1e-4, debug_every=5000, prefix=None):
    ...
    if prefix is not None: # Création du dossier de log
    ...
    for epoch in range(n_epochs):
        ...
        # Enregistrement des métriques d'entraînement
        history["train_loss"].append(epoch_loss / len(train_loader))
        history["train_acc"].append(train_correct / train_total)

        # Évaluation du modèle sur l'ensemble de validation et enregistrement des métriques
        val_acc, val_loss = evaluate_mil_model(model, val_loader, device, criterion) # Récupération de la perte de validation
    ...
    # Sauvegarde des historiques à la fin de l'entraînement
