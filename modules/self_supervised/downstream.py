...
from modules.self_supervised.siamese import SiameseDataLoader, SiameseDataset, SiameseModel, triplet_loss, DataAugmentations
...
def handle_train_ddp(rank, world_size, proportion, load_supervised_model):
    ...
    unlabeled_train_size = int(0.9 * len(unlabeled_set))
    unlabeled_val_size = len(unlabeled_set) - unlabeled_train_size
    unlabeled_train_set, unlabeled_val_set = random_split(unlabeled_set, [unlabeled_train_size, unlabeled_val_size])

    if rank == 0:
        print("--- Upstream SSL Pre-training ---")
        print(f"Unlabeled subset for SSL (proportion {100-proportion*100:.2f}%): {len(unlabeled_set)} samples")
        print(f"  -> SSL training set size: {len(unlabeled_train_set)}")
        print(f"  -> SSL validation set size: {len(unlabeled_val_set)}")
    ...
    tower_model = MsAAGCN(num_joint=NUM_JOINT, num_skeleton=NUM_SKELETON, edge_index=EDGE_INDEX, in_channels=IN_CHANNELS, latent_size=LATENT_SIZE).to(rank)

    warm_start_successful = False
    if load_supervised_model:
        # La logique de chargement pour un warm-start reste la même
        pass # Conservez votre logique de chargement ici
    ...
    trainer = SSLTrainer(model=siamese_model, optimizer=optimizer, loss_function=loss_function,
                         batch_size=BATCH_SIZE, rank=rank, world_size=world_size, device=device, patience=PATIENCE)
    ...
    try:
        ...
        if rank == 0:
            print(f"Upstream training completed in {end_time - start_time:.2f} seconds")
            print(f"Best tower model saved in logs/{ssl_prefix}/best_siamese_tower.pt")
    finally:
        cleanup_ddp()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--proportion", type=float, default=0.1, help="Proportion of the dataset to be considered 'labeled' and thus excluded from this pre-training. Ex: 0.1")
    parser.add_argument("--load-supervised-model", action="store_true", help="Load a pre-trained supervised model for warm-starting the tower model.")
